...
export default function PostulacionesDocentesIndex({ postulaciones, filtros }) {
    const [busqueda, setBusqueda] = useState(filtros.busqueda);
    const [estado, setEstado] = useState(filtros.estado);

    const aplicarFiltros = (e) => {
        e.preventDefault();
        router.get('/admin/postulaciones-docentes', {
            busqueda: busqueda || '',
            estado: estado || '',
        }, { preserveState: true });
    };
    ...
    const getEstadoBadge = (est) => {
        ...
        return colores[est] || 'bg-slate-100 text-slate-700 border-slate-200';
    };
    ...
    return (
        <AdminLayout>
            <Head title="Postulaciones Docentes" />
            ...
            <div className="max-w-7xl mx-auto">
                <div className="flex items-center gap-4 mb-8 pb-6 border-b border-slate-200">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Postulaciones Docentes</h1>
                        <p className="text-sm text-slate-500 mt-1">Revisa y gestiona las postulaciones recibidas</p>
                    </div>
                </div>
                ...
                <form onSubmit={aplicarFiltros} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label className="mb-1.5 block text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Buscar</label>
                        ...
                        <div>
                            <label className="mb-1.5 block text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Estado</label>
                        ...
                        <div className="flex items-end gap-2">
                            <button type="submit" className="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-colors">Buscar</button>
                            <button type="button" onClick={limpiarFiltros} className="px-4 py-2 border border-slate-300 rounded-lg text-sm font-medium text-slate-600 bg-white hover:bg-slate-50 transition-colors">Limpiar</button>
                        </div>
                    </div>
                </form>
                        ...
                        <div className="p-16 text-center text-slate-500">
                            <div className="bg-slate-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                                <svg className="w-8 h-8 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857" />
                                </svg>
                            </div>
                            ...
                            <table className="w-full text-left text-sm">
                                <thead>
                                    <tr className="bg-slate-50/80 border-b border-slate-200">
                                        ...
                                        <th className="px-6 py-4 font-semibold text-slate-600 uppercase tracking-wider text-[10px]">Materias</th>
                                        ...
                                        <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex flex-wrap gap-1">
                                                    {p.materias && p.materias.length > 0 ? p.materias.map((m, i) => (
                                                        <span key={i} className="text-[10px] font-medium text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md border border-blue-100">{m.nombre}</span>
                                                    )) : <span className="text-xs text-slate-400">—</span>}
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-xs text-slate-500">{formatearFecha(p.fecha_postulacion)}</td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className={`inline-block text-[10px] font-semibold px-2.5 py-1 rounded-full border ${getEstadoBadge(p.estado_postulacion)}`}>
                                                    {p.estado_postulacion}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <Link href={`/admin/postulaciones-docentes/${p.id}`}
                            ...
                            <div className="flex gap-1">
                                {postulaciones.links.map((link, idx) => (
                                    <Link key={idx} href={link.url || '#'}
                                        className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-colors ${link.active ? 'bg-blue-600 text-white' : link.url ? 'bg-slate-100 text-slate-600 hover:bg-slate-200' : 'bg-slate-50 text-slate-300 cursor-not-allowed'}`}
